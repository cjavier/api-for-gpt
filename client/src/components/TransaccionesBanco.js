...
export default function TransaccionesBanco() {
  ...
  useEffect(() => {
    const fetchFacturas = async () => {
      ...
      const facturasRef = collection(db, `usuarios/${currentUser.uid}/transacciones`);
    ...
    <Layout>
      <Grid item xs={12}>
        <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column' }}>
          <h1>Lista de Transacciones del Banco</h1>
          <Box sx={{ mt: 2 }}>
            {isLoading ? (
              <CircularProgress />
            ) : (
                  ...
                  <TableBody>
                    {facturas.map((factura) => (
                      <TableRow key={factura.id}>
                        ...
                        <TableCell align="right">{factura.date.toDate().toLocaleDateString()}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
